...
def inputToFileData(Data):
    f=open("Data.txt","w")
    f.write(str(Data))
    #print("inserting the counter",Data)
    f.close()
   
def inputToFileValue(Data):
    f=open("Value.txt","w")
    f.write(str(Data))
    f.close()
...
time.sleep(2.0)
#pic=1
Counter=0 # average of face detection in 30 sec
Value=0 # determins if there is a face or no
...
#     if StopLoop==1:
#         time.sleep(5)
#         Counter=0 # average of face detection in 10 sec
#         Value=0 # determins if there is a face or no
#         inputToFileData(Counter)
#         inputToFileValue(Value)
...
#     cleanup=0
    Time=getFromFileTimer()
#     print("out of loop ",cleanup)
...
#             cleanup=1
            ...
            # face mask or not
            (locs, preds,LenFaces) = detect_and_predict_mask(frame, faceNet, maskNet)
        ...
        # 			takePic()
        # 			pic=1
                #label = "Mask" if mask > withoutMask else 
